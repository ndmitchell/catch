...
An expression in our first-order core language either evaluates to a (potentially infinite) data structure, or to $\bot{}$ caused by either non-termination of pattern match error. If an expression does evaluate to a data structure, then a constraint states the possible forms of data value it may take. A constraint describes a set of data structures that a value may match. Two alternative constraint systems will be introduced in \S\ref{sec:regexp} and \S\ref{sec:multipattern}.
