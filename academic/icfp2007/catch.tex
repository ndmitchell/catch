...
In Haskell,type classes are typically implemented as dictionaries \citep{wadler:type_classes}. All functions with qualified polymorphic types have one or more dictionaries of class methods as auxiliary arguments. This means that the function to analyse must not have any outstanding context.
...
The next step where many fail is that \C{read} is applied to an argument extracted from getArgs. Firstly this argument is entirely unknown. Secondly \C{read} is a sufficiently complicated function that while it can be modeled by Catch, there is no possibility for getting an appropriate abstraction which models the failure case. As a result, any program which calls \C{read} is unsafe, according to Catch. Using \C{reads}, which indicates failure properly, can allow a program to still be checked.
...
There have been attempts at defining what it means for a program to be complete with regard to pattern matches \citep{maranget:pattern_warnings}, along with implementations in some compilers.

One approach for programming is to design a functional language without pattern match errors or non-termination \cite{turner:total}. In this approach incomplete pattern matches are banned, suggesting this will ``force you to pay attention to exactly those corner cases which are likely to cause trouble''. The results in \S\ref{sec:nofib} show that in sample Haskell programs there are a reasonably high number of incomplete pattern matches, which are still safe. Using the Catch tool this restriction could be reduced.

\subsection{A Mistake Detector}

One tool in most Haskell programmers arsenal is the QuickCheck tool \citep{quickcheck}. This tool allows properties to be specified which are randomly tests. In many cases these properties may be similar to those derived by the templating mechanism -- if the Catch tool had a way of directly entering these conditions then perhaps some of the simpler properties could be automatically proven.
...
The type directed method can be pushed much further with dependant types, which allow types to depend on values. This approach allows safe versions of \C{head} and \C{tail} to be written. There has been much work on type systems, some using undecidable type systems \citep{cayenne, epigram}, some based on extensible kinds \citep{omega} and others which restrict the type system to a decidable fragment \citep{xi:dependent_practical}.
