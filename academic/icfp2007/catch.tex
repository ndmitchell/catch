...
\textbf{Err} is the number of calls to \C{error}.
\textbf{Pre} is the number of functions which have a precondition which is not True.
\textbf{Sec} is the time in seconds taken for transformations and analysis, excluding the standard Yhc compilation time. No great effort has been put into the optimisation of the analysis code.
\textbf{Mb} is the maximum residency of the program at garbage collection time, in Mb. Typically a program will consume twice this size in system memory.

\smallskip

\begin{tabular*}{\linewidth}{lrrrrrrlll}
Program               & Src & Core & First & Err & Pre & Sec & Mb \\
\vspace{-2.2ex} \\
\hline
\vspace{-1.8ex} \\
