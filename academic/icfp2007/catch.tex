...
The initial challenge to checking a library is that there is no \C{main} function, but wrappers are provided such that:

\begin{code}
main = function1 ||| function2 ||| function3
\end{code}

The |(||||||)| operator takes two functions, and using some Haskell type classes saturates them with enough arguments, then checks that a crash does not result. This operator is provided in an additional library by \catch{}.
...
Here \catch{} is unable to prove that this function is safe. By changing \C{isWindows} to \C{otherwise}, this code can be proven safe. The function is safe, but relies on the CAF \C{System.Info.os} being constant through the run of the program -- something \catch{} does not currently do.

The one unfortunate restriction when checking libraries is that all interfaces must be first order and have no outstanding context, this is often quite restrictive.
