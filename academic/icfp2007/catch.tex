...
The best way to see the power of Catch is by example. While introducing the constraints in \S\ref{sec:small_examples} I gave a few micro-examples. \S\ref{sec:imaginary} investigates examples from the Imaginary section of Nofibe, \S\ref{sec:spectral} gives results from larger examples chosen from the Spectral section of Nofib.

\subsection{The Price of Safety}

\todo{Redraft this section}

Before discussing the failing programs, it is useful to reflect on a small example:
...
In general pattern match errors can either be worked around locally, close to where the pattern match error may occur, or on a more global basis -- in the callers of the unsafe function. The most pleasing result is likely to be achieved using a global fix, but the local fix is likely to be easier to implement.

In the following sections, where modifications were required, we have used local fixes. In some cases the program is not safe without some fix -- the whole program is likely to change substantially.
...
We will now discuss the 4 programs which required changes, along with the Digits of E2 program -- a program with complex pattern matching that \catch{} is able to prove safe.
