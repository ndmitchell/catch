...
The contributions beyond the original pattern match checker paper are many and varied. List them here.
...
notin :: CtorName -> Constraint
notin c = (map (/= c) cs :| as) :* (replicate (length cs) True :| as)
    where
    cs = ctors c; xs = fields cs
    as = replicate (length xs) Any

(|>) :: PathName -> Constraint -> Constraint
p |> vs = notin c ++ map f vs
    where
    c = pathCtor p; cs = ctors c
    r = isRec p; xs = fields cs

    f Any = Any
    f (v1 :* v2) | r =  (map (== c) cs :| replicate (length xs) Any) :*
                        (v1 `intersect` v2)
    f v =  (map (== c) cs :| fs) :*
           (replicate (length cs) True :| replicate (length xs) Any)
        where fs = map (lambda x -> if x == p then v else Any) xs
...
Each constraint has two parts, the first part (before the |:*|) represents the constructor and the non-recursive fields. The second part represents all recursive fields. Given a constraint |((c_1 :* v_1) :|| (c_2 :* v_2))|, each |c| is a set of constructors, and each |v| is a mapping from non-recursive field names to values. In this example, |c_1| is the set of constructors allowed in the value, and |c_2| is the set allowed after following \textit{any number} of recursive fields. Similarly, |v_1| represents the fields allowed following non-recursive field names from the root, while |v_2| lists the field names from the recursive values.
...
This constraint states that the first constructor is a |(:)|, note that it is a lot more verbose than the regular expression based one.

\begin{code}
(["[]",":"] :| [("hd",cons)]) :* (["[]",":"] :| [("hd",cons)])
\end{code}

Here I have used the constraint from the previous time to abbreviate the \C{cons} requirement.
