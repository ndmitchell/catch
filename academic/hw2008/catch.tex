...
pre (Call  f  xs     ) = pre' `propAnd` propAnds (map pre xs)
    where pre'= substP (zip (args f) xs) (precond f)
...
red (Call  f  xs     ) k =  reduce $ substP xs (prePost f k)
