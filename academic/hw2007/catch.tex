...
We have implemented all the techniques reported here. We encourage readers to download the Catch tool and try it out. It can be obtained from the website at \url{http://www.cs.york.ac.uk/~ndm/catch/}. A copy of the library has also been released, and is available on Hackage\footnote{\url{http://hackage.haskell.org/}}.
...
\begin{figure}
\begin{code}
data Prop alpha = Prop alpha

propAnd      :: Prop alpha -> Prop alpha -> Prop alpha
propOr       :: Prop alpha -> Prop alpha -> Prop alpha
propAnds     :: [Prop alpha] -> Prop alpha
propOrs      :: [Prop alpha] -> Prop alpha
propMap      :: (alpha -> beta) -> Prop alpha -> Prop beta
propImp      :: Prop alpha -> Prop alpha -> Prop alpha

propTrue     :: Prop alpha
propFalse    :: Prop alpha
propBool     :: Bool -> Prop alpha
propIsTrue   :: Prop alpha -> Bool
propIsFalse  :: Prop alpha -> Bool
...
Atomic constraints can be combined into propositions, using the proposition data type in Figure \ref{fig:prop}. In addition to standard logical operators such as |propAnd| and |propOr|, we also use |propAnds| and |propOrs|, equivalent to |and| and |or| in the Haskell Prelude. We also make use of |propMap| which modifies every single terminal. We use |propTrue|/|propIsTrue| to constructor a true proposition, and to test is a proposition is true. Finally, the |propLit| function lifts a single atomic constraint into a proposition on that constraint. We also introduce |(==>)|, standard boolean implication, which Haskell already provides with |(<=)|, but we prefer the clarity of a different symbol.
...
propRestrictive fn xs = propIsTrue $
    preFunc fn xs `propImp`
    pre (instantiate fn xs)
\end{code}
...
(c,i) |> k = map extend k
    where
    extend x = Con c (anys i ++ [x] ++ anys (n-i-1))
...
\begin{revisit}{1} safe evaluation of |(head xs)| requires |xs| to be non-empty. The MP-constraint generated by \catch{} on |xs| is: \ignore|{(:) Any} :* {[], (:) Any}| This constraint is longer than the corresponding RE-constraint as it makes explicit that the both the \C{hd} field and the recursive components are unrestricted.
\end{revisit}
...
Examining the |read| error in more detail, by default \catch{} outputs the potential error message, and a list of incomplete function in a call stack:

\begin{verbatim}
Checking: "Prelude.read: no parse"
Partial: "Prelude.read$252"
Partial: "HsColour.Colourise.parseColourPrefs"
Partial: "YHC.Internal.IO.>>=$254"
Partial: "HsColour.Colourise.readColourPrefs"
...
\end{verbatim}

We can see (ignoring the text after \texttt{\$}) that the call stack would be |read|, then \ignore|parseColourPrefs|, and so on. By using our knowledge of the intention of functions, we can see that although |read| calls error, the blame probably lies in the caller of |read| -- namely \ignore|parseColourPrefs|. By examining this location in the source code we are able to diagnose and correct the problem. \catch{} does produce constraints, although in our experience the problem can usually be fixed from the source position information.
