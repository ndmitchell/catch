...
\item A \textbf{precondition} is a predicate of constraints on the arguments to a function, to ensure the result of the function will not contain $\bot{}$.
\item A \textbf{property}     is a predicate of constraints on the arguments to a function, to ensure the result of the function will satisfy a further constraint.
\end{itemize}
...
The syntax for our core language is given in Figure \ref{fig:core}. Our Core language is a little more restrictive than the core languages typically used in compilers \cite{ghc_core}. It is first order, has only simple case statements, and only algebraic data types. All case statements have alternatives for all constructors, with |error| calls being introduced where a pattern-match error would otherwise occur.
...
Figure \ref{fig:core_operations} gives the signatures for helper functions over the core data types. Every constructor has an arity, which can be obtained with the |arity| function. To determine alternative constructors the |ctors| function can be used; for example |ctors "True" = ["False", "True"]| and |ctors "[]" = ["[]",":"]|. The |var| function returns |Left i| for a variable bound as the |i|th argument of a top-level function, and |Right (e,(c,i))| for a variable bound as the |i|th component in the |c|-constructed alternative of a case-expression whose scrutinee is |e|. The function |instantiate| corresponds to $\beta$ reduction. The |isRec (c,i)| function returns true if the constructor |c| has a recursive |i|th component; for example, let |hd = (":",0)| and |tl = (":",1)| then |isRec hd = False| but |isRec tl = True|.

\subsubsection{Transformation From Haskell to Core}
\label{sec:transform}

To generate core representations of programs, we start with the York Haskell Compiler (Yhc) \citep{me:yhc_core}. Yhc can transform a Haskell program into a single Yhc Core file, including all necessary libraries. Yhc Core is higher-order and contains let expressions, both of which are absent in our Core language. The let expressions can be removed by simple transformations provided by the Yhc Core library.
...
The Core language only has algebraic data types. Catch allows for primitive types such as characters and numbers by abstracting them into algebraic types. Two abstractions used in Catch are:
...
We write |x :< c| to assert that the value of expression |x| must be a member of the set described by the constraint |c|, i.e. that |x| \textit{satisfies} |c|. If any component of |x| evaluates to $\bot{}$, the constraint is vacuously satisfied. Atomic constraints can be combined into propositions, using the proposition data type in Figure \ref{fig:prop}.
...
For simplicity, our analysis framework will be introduced using basic pattern constraints (BP-constraints). A BP-constraint is represented as in Figure \ref{fig:basic}, and corresponds to Haskell pattern matching, where |Any| represents an underscore. A given data structure |d| satisfies a BP-constraint |p| if it matches the pattern. The requirement for a value to be |(:)|-constructed would be expressed as |(Con ":" [Any,Any])|. The BP-constraint language is limited in expressivity, for example it is impossible to state that all the elements of a boolean list are True.
...
\begin{figure}
\begin{code}
pre ::  (FuncName -> [Expr] -> Prop (Assert Expr)) ->
        Expr -> Prop (Assert Expr)
...
\begin{code}
pf null [xs] `propAnd` (null xs `is` ["True"] `propOr` pf tail [xs])
\end{code}
...
\begin{figure}
\begin{code}
preVars  :: FuncName            -> Prop (Assert Expr)
preFun   :: FuncName -> [Expr]  -> Prop (Assert Expr)

preVars Rec0   fn = if fn == "error" then false else true
preVars RecN1  fn = preVars RecN fn `propAnd` pre preFun RecN (instantiate fn vs_)

preFun RecN fn xs_ = (preVars RecN fn)[vs_ / xs_]

where vs_ = -- list of free variables, of length |arity fn|
\end{code}
\begin{comment}
\ignore\begin{code}
...
The iterative algorithm for calculating preconditions is given in Figure~\ref{fig:precond_fixp}. The |preVars| function calculates the precondition on a function given a list of variables as arguments, while |preFun| replaces these variables with whatever arguments are desired. Initially all preconditions are assumed to be true, apart from the |error| precondition, which is false. We take the fixed point of |preFun| to be the definitive preconditions. In each iteration of |preVars| the preconditions become more restrictive. So \textit{if all chains of increasingly restrictive constraints are finite}, termination is guaranteed -- a topic we return to in \S\ref{sec:bounded}.
...
\begin{figure}
\begin{code}
backs ::  (FuncName -> Constraint -> Prop (Assert ArgPos)) ->
          Prop (Assert Expr) -> Prop (Assert ArgPos)
backs property = propMap back
where

back :: Assert Expr -> Prop (Assert ArgPos)
back (Var   x         :< k) =  case var x of
                               Left   i       -> propLit $ i :< k
                               Right  (on,c)  -> back $ on :< (c |> k)
back (Make  c   xs    :< k) = merge xs (c <| k)
back (Call  fn  xs    :< k) = merge xs (property fn k)
back (Case  on  alts  :< k) = propAnds [f c e | Alt c vs e <- alts]
    where f c e = propMap back (on `is` (ctors c \\ [c])) `propOr` back (e :< k)

merge ::  [Expr] -> Prop (Assert ArgPos) -> Prop (Assert ArgPos)
merge xs p = propMap (\(i:<k) -> back $ (xs!!i) :< k) p
\end{code}
\caption{Specification of backward analysis, |back|.}
\label{fig:backward}
\end{figure}

The precondition calculation generates a predicate in terms of subexpressions of the function body. We simplify this precondition to be in terms of the functions arguments only, allowing the predicate to become simpler -- and hopefully discharged.
...
Constraints are rewritten using the |backs| function, given in Figure \ref{fig:backward}. The result is a predicate where all assertions are over the functions arguments, denoted by argument position. The |merge| function takes a predicate of constraints over argument positions, and a list of arguments supplied, and merges the arguments into the predicate.
...
\begin{code}
c <| Any = propTrue
c <| Con c_2 xs  =          propBool (c_2 == c)
                 `propAnd`  propAnds (map propLit (zipWith (:<) [0..] xs))
\end{code}
...
":" <| Con ":" [Con "True" [],Any] = propLit $ 0 :< Con "True" []
":" <| Con ":" [Any,Con "[]" []] = propLit $ 1 :< Con "[]" []
\end{code}

\para{The |Case| rule} generates a conjunct for each alternative. An alternative is safe if \textit{either} it is never taken, \textit{or} it meets the constraint when taken. The |notin| function from Figure \ref{fig:precondition} is used to assert that a particular alternative never matches.
...
property RecN1  fn k =  property RecN fn k `propAnd`
                        backs property RecN (propLit $ instantiate fn vs_ :< k)

where vs_ =  -- free variables, which |var| treats
             -- as successive function arguments
\end{code}
\begin{comment}
...
\para{The |Call| rule} relies on the |property| function, defined in Figure \ref{fig:property_fixp}. A property is a mapping from a postcondition on a function, to a predicate of constraints on the arguments to that function -- a precondition to ensure the postcondition. Initially, the assumption is that all arguments to any function will guarantee any constraint. To calculate the property for |fn| to result in a value satisfying |k|, the iterative step takes the body of |fn| with appropriate parameters substituted, and uses the |backs| transformation to obtain a predicate in terms of the arguments to the function only. As in the precondition calculation in \S\ref{sec:precond}, each result becomes increasingly restrictive. If refinement chains of constraint/function pairs are finite, termination is guaranteed. Here again, a speed up can be obtained by tracking the dependencies between constraints, and additionally caching all calculated properties.
...
satisfies (Value c xs)  k = propMap f (c <| k) == propTrue
    where f (i :< k') = propBool $ satisfies (xs !! i) k'
\end{code}
\end{minipage}
...
propExtend  v@(Value c xs) k i =
    not (satisfies v ((c,i) |> k)) || satisfies (xs !! i) k
...
\todo{Insert examples of all the operations on regular expressions}
...
is :: :: alpha -> [CtorName] -> Prop (Assert alpha)
is e [] = propFalse
is e cs = propLit $ e :<  [map complete cs :* map complete (ctors (head cs))]
...
\todo{Expand on the descriptions}

\begin{examplerevisit}{\ref{ex:head}} Safe evaluation of |(head xs)| requires |xs| to be non-empty. The MP-constraint generated by Catch on |xs| is: \ignore|{(:) Any} :* {[], (:) Any}| This constraint is longer than the corresponding RE-constraint as it makes explicit that both the |hd| field and the recursive components are unrestricted.
\end{examplerevisit}
...
\todo{Insert some examples of the operations, including an intuition}
...
XMonad \cite{xmonad} is a window manager, which automatically manages the layout of program windows on the screen. The central module of XMonad contains a pure API, which is used to perform manipulate a data structure containing information regarding window layout. Catch has been run on their central module, several times, as the design has evolved. Their API contains 36 exported functions, most of which are intended to be total. Within the implementation of these functions, there are a number of incomplete patterns and calls to partial functions.

When the Catch tool was first used, it detected six separate issues which were causes for concern -- including unsafe uses of partial functions, API functions which contained incomplete pattern matches, and unnecessary assumptions about the |Ord| class. All these issues were subsequently fixed. While XMonad is a program, Catch is being used to check API functions, ensuring they are complete. The XMonad developers have said: ``QuickCheck and Catch can be used to provide mechanical support for developing a clean, orthogonal API for a complex system.''
...
The closest other work we are aware of is ESC/Haskell \cite{esc_haskell} and its successor Sound Haskell \cite{xu:sound_haskell}. The Sound Haskell approach requires the programmer to give explicit preconditions and contracts which the program obeys. Contracts have more expressive power than our constraints -- one of the examples involves an invariant on an ordered list, something beyond Catch. But the programmer has more work to do. At the time of writing there is no publicly available version of Sound Haskell. So far as we know, it does not yet support full Haskell, lacks features such as type classes\footnote{We have confirmed this information in correspondence with Xu.}, and handles only small examples. A future version of Sound Haskell will hopefully address these limitations, allowing a fuller comparison to be made.
