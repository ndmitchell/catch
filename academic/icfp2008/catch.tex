...
\begin{code}
is a xs = propOrs [propLit $ a :< Con x (replicate (arity x) Any) | x <- xs]
\end{code}
...
e `is` [":","[]"]  = propLit (e :< Con ":" [Any,Any]) `propOr` propLit (e :< Con "[]" [])
\end{code}
...
red :: Expr -> Constraint -> Prop (Assert ArgPos)
...
satisfiesP x = propMap (\(v :< k) -> propBool $ satisfies v k) x == (propTrue :: Prop ())
\end{code}
\end{minipage}
