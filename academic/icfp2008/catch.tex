...
Three operations must be provided by every constraint model. Signatures are given in Figure \ref{fig:constraint}. |Assert| is the type of the |(:<)| condition. The lifting and splitting operators |(||>)| and |(<||)| are discussed in \S\ref{sec:backward}. The |is| function generates a constraint which is only satisfied by values whose root constructor is one of the given constructors. All applications of |is| must ensure that at least one constructor is given.
...
\begin{code}
is xs = [Con x (replicate (arity x) Any) | x <- xs]
\end{code}
...
pre pf (Case  on  alts  ) = pre pf on `propAnd` propAnds (map f alts)
    where f (Alt c vs e) = notin on c `propOr` pre pf e

notin x c = if null cs then true else propLit (x :< is cs)
    where cs = delete c (ctors c)
\end{code}
...
\begin{code}
pf null [xs] `propAnd` (propLit (null xs :< is ["True"]) `propOr` pf tail [xs])
\end{code}
...
\begin{figure}
%format preVars_0 = "\Varid{preVars_0}"
%format preVars_n = "\Varid{preVars_n}"
%format preVars_n_1 = "\Varid{preVars_{n\text{\tiny{+}}1}}"
%format preFun_n = "\Varid{preFun_n}"
\begin{code}
preVars  :: FuncName            -> Prop (Assert Expr)
preFun   :: FuncName -> [Expr]  -> Prop (Assert Expr)

preVars_0    fn = if fn == "error" then false else true
preVars_n_1  fn = preVars_n fn `propAnd` pre preFun_n (instantiate fn vs_)

preFun_n fn xs_ = (preVars_n fn)[vs_ / xs_]
...
back (Make  c   xs    :< k) = replacePos xs (c <| k)
back (Call  fn  xs    :< k) = replacePos xs (property fn k)
back (Case  on  alts  :< k) = propAnds [f c e | Alt c vs e <- alts]
    where f c e = propMap back (notin on c) `propOr` back (e :< k)

replacePos ::  [Expr] ->
               Prop (Assert ArgPos) -> Prop (Assert ArgPos)
replacePos xs p = propMap (\(i:<k) -> back $ (xs!!i) :< k) p
\end{code}
...
\begin{code}
propLit (null xs :< is ["True"]) `propOr` propLit (xs :< is [":"])
\end{code}

\noindent We can now use the property that if the result of |null| is |True|, then the argument must have been |[]| to write:

\begin{code}
propLit (xs :< is ["[]"]) `propOr` propLit (xs :< is [":"])
\end{code}

\noindent Which can be shown to be a tautology.
\end{examplerevisit}

Constraints are rewritten using the |backs| function, given in Figure \ref{fig:backward}. The result is a predicate where all assertions are over the functions arguments, denoted by argument position. The |replacePos| function takes a predicate of constraints over argument positions, and a list of arguments supplied, and substitutes the arguments into the predicate.
...
\begin{code}
(c,i) |> k = map extend k
    where extend x = Con c  [if i == j then x else Any
                            | j <- [0..arity c - 1]]
\end{code}
...
Here the initial precondition will be |y :< is [":"]|, which evaluates to |[Con ":" [Any,Any]]|. The |var| function on |y| will return |Right (xs,(":",0))|. After the application of | ||>| the result will be |xs :< [Con ":" [Con ":" [Any,Any], Any]]|. We have gone from a constraint on |y|, using the knowledge that |y| is bound to a portion of |xs|, to a constraint on |xs|.
\end{example}
...
\begin{figure}
%format property_0 = "\Varid{property_0}"
%format property_n = "\Varid{property_n}"
%format property_n_1 = "\Varid{property_{n\text{\tiny{+}}1}}"
\begin{code}
property :: FuncName -> Constraint -> Prop (Assert ArgPos)
property_0    fn k = propTrue
property_n_1  fn k =  property_n fn k `propAnd`
                      backs property_n (propLit $ instantiate fn vs_ :< k)
...
satisfies (Value c xs)  k = propIsTrue $ propMap f (c <| k)
    where f (i :< k_2) = propBool $ satisfies (xs !! i) k_2
\end{code}
\end{minipage}
...
is :: [CtorName] -> Constraint
is cs = [map complete cs :* map complete (ctors (head cs))]
