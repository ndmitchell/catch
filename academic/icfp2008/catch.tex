...
property  :: FuncName -> Prop (Assert ArgPos)
...
The type signatures for the functions providing preconditions and properties are given in Figure \ref{fig:operations}. Both take a function name, and give a predicate of assertions over the arguments to that function, denoted by argument position. During the manipulation of constraints, we will often wish to talk about constraints on expressions, rather than argument positions. We will require the |reduce| function, which converts predicates on expressions to predicates on arguments. We also define the |substP| function which replaces constraints on argument positions with a sequence of arguments.
...
pre (Call  fn  xs    ) = substP (precond fn) xs `propAnd` propAnds (map pre xs)
...
red :: Assert Expr -> Prop (Assert ArgPos)
...
Constraints are rewritten using the |reduce| function, given in Figure \ref{fig:backward}. The result is a predicate where all assertions are over the functions arguments, denoted by argument position.
