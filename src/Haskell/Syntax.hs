...
module Haskell.Syntax (
    ...
    ModName(..), HsQName(..), HsName(..), HsQOp(..), HsOp(..),
...
-- The following instance is slightly modified for Haskell to Clean
-- translator (Hacle).
-- I would have thought these instances should be in the pretty printer
-- rather than the syntax description...
instance Show HsSpecialCon where
    show HsUnitCon = "Trivial"
    show HsListCon = "[]"
    show HsFunCon = "->"
...
instance Show HsQName where
   showsPrec _ (Qual (ModName m) (HsIdent s))
     | isUpper (head s) =
       showString "Id_" . showString m . showString "_" . shows s
     | otherwise =
       showString "id_" . showString m . showString "_" . shows s
   showsPrec _ (Qual (ModName m) (HsSymbol s)) =
     showString "sym_" . showString m . showString "_" . shows (removeSyms s)
     where removeSyms = concatMap (('_':) . show . ord)
...
data HsModule = HsModule SrcLoc ModName (Maybe [HsExportSpec])
                         [HsImportDecl] [HsDecl]
  deriving Show
...
data HsExportSpec
     ...
     | HsEModuleContents ModName        -- ^ @module M@:
            -- re-export a module.
  deriving (Eq,Show)

-- | Import declaration.
data HsImportDecl
     = HsImportDecl SrcLoc ModName Bool (Maybe ModName)
...
data HsDecl
     ...
     | ClTypeSig     SrcLoc [HsName] HsQualType (Maybe (HsAssoc, Int))
     | HsFunBind     [HsMatch]
     | HsPatBind     SrcLoc HsPat HsRhs {-where-} [HsDecl]
   | ClMacroDecl HsName [HsPat] HsExp
  deriving (Eq,Show)
...
data HsConDecl
   ...
   | ClConDecl SrcLoc HsName [HsBangType] (Maybe (HsAssoc, Int))
...
data HsType
     ...
     | HsTyVar   HsName
     | HsTyCon   HsQName
   | HsUncurriedTyFun HsType HsType
   | HsParenTy HsType
  deriving (Eq,Show)
...
prelude_mod, main_mod :: ModName
prelude_mod       = ModName "Prelude"
main_mod          = ModName "Main"
