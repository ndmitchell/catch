...
import Haskell.ParseMonad
import Haskell.Lexer
import Haskell.ParseUtils
...
happyReduction_254 (HappyTerminal (QVarId happy_var_1))
         =  HappyAbsSyn42
                 (Qual (ModName (fst happy_var_1)) (HsIdent (snd happy_var_1))
        )
...
happyReduction_263 (HappyTerminal (QConSym happy_var_1))
         =  HappyAbsSyn42
                 (Qual (ModName (fst happy_var_1)) (HsSymbol (snd happy_var_1))
        )
...
happyReduction_284 (HappyTerminal (QConId happy_var_1))
         =  HappyAbsSyn127
                 (ModName (fst happy_var_1 ++ '.':snd happy_var_1)
        )
...
-- $Id: Parser.hs,v 1.1 2005/11/05 22:15:53 ndm Exp $
