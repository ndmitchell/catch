...
runi :: (Eq a, Show a) => [Regex a] -> Regex a
...
rcon :: [Regex a] -> Regex a -> Regex a
rcon [] b = b
rcon a (RCon x) = RCon (a++x)
rcon a b = RCon (a++[b])

rcon2 :: Regex a -> [Regex a] -> Regex a
rcon2 a [] = a
