...
clauses x = concat (map disp (unicl (split (disin (negin (elim x)))))) -- . parse
...
disin (Con p q) = Con (disin p) (disin q)
disin (Dis p q) = disin2 (disin p) (disin q)
disin p = p

disin2 (Con p q) r = Con (disin2 p r) (disin2 q r)
disin2 p (Con q r) = Con (disin2 p q) (disin2 p r)
disin2 p q = Dis p q
...
tautclause (c,a) = not (null [x | x <- c, x `elem` a])
