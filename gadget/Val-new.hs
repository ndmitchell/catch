...
data Val = Val Part (Maybe Part)
         | Any
         deriving (Ord,Eq)


data Part = Part [(CoreCtorName, Bool, [Val])]
            deriving (Ord,Eq)
...
instance Show Val where
    ...
    show (Val a b) = show a ++ maybe "" ((" * "++) . show) b


instance Show Part where
    show (Part xs) = concat $ intersperse " + " $ map f xs
        where
            f (name,_,fields) = unwords (name:map g fields)
            g Any = "_"
            g x = "(" ++ show x ++ ")"
...
valsOr core [] b = b
valsOr core a (b:bs) = valsOr core (valOr core a b) bs
valsOr core a [] = a


valsOrs :: Core -> [Vals] -> Vals
valsOrs core (x1:x2:xs) = valsOrs core (valsOr core x1 x2 : xs)
valsOrs core [x] = x
valsOrs core [] = []
...
normalise core xs = valsOrs core $ map (:[]) xs
...
checkRoot core vals name = any f vals
    where
        f Any = True
        f (Val (Part xs) _) = name `elem` map fst3 xs



integrate :: Core -> Vals -> CoreFieldName -> Vals
integrate core vals field = undefined
{-
        | Any `elem` vals = [Any]
...
differentiate core name vals = undefined {-
        | null vals = []
